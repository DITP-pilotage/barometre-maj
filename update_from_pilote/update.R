...
r%>%filter(indic_id=="IND-200" & enforce_zone_id=="R06" & metric_enforce_date=="2023-03-31")

r%>%filter(indic_id=="IND-200" & enforce_zone_id=="R06" & metric_enforce_date=="2023-03-31") %>%
  group_by(indic_id, enforce_zone_id, metric_enforce_date) %>% count(as.character(coalesce(is_hist, F))=="bob")
